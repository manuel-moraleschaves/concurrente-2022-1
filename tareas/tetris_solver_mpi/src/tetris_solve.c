...
void solve_tetris_omp(tetris_t* base_tetris, struct level_t* base_level,
    ...
    for (int col = 0; col < base_tetris->columns; col++) {
        for (int rotation = 0; rotation < num_rotations; ++rotation) {
            ...
            {
                tetris = clone_tetris(base_tetris, base_level);
            }

            figure_t* figure = get_tetris_figure(tetris->figure_sequence[1],
                                                 rotation);
            ...
            if (valid_column(tetris, figure, col)) {
                ...
                if (num_row != -1) {
                    // Crea el registro del nivel
                    struct level_t* level =
                                        create_level(tetris->figure_sequence[1],
                                        rotation, tetris->rows, tetris->columns,
                                        tetris->matrix);
                    if (level) {
                        // Agrega el nuevo nivel
                        tetris->levels->next->next = level;
                        tetris->levels->next->next->next = NULL;
                    }

                    // Llamado a rutina recursiva con la siguiente figura
                    solve_tetris_dfs(tetris, 2, tetris->levels, base_tetris);
                }
            }
            // Liberar tetris
...
void solve_tetris_mpi(tetris_t* tetris, int thread_count, int rank,
    ...
    for (int col = rank; col < tetris->columns; col+=process_count) {
        for (int rotation = 0; rotation < num_rotations; ++rotation) {
            ...
            // Se valida si la figura cabe en la columna
            if (valid_column(tetris, figure, col)) {
                // Crea la estructura de niveles
                struct level_t* base_level = create_level('B', 0, tetris->rows,
                                             tetris->columns, tetris->matrix);
                ...
                if (num_row != -1) {
                    // Crea el registro del nivel
                    base_level->next = create_level(tetris->figure_sequence[0],
                                        rotation, tetris->rows, tetris->columns,
                                        tetris->matrix);
