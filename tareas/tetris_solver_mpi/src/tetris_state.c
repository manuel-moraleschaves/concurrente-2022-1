...
tetris_t* clone_tetris(tetris_t* base_tetris, struct level_t* base_level) {
    ...
    tetris->levels = create_level('B', 0, tetris->rows, tetris->columns,
                                  tetris->matrix);
    if (base_level->next) {
        tetris->levels->next = create_level(base_level->next->figure,
            base_level->next->rotation, tetris->rows, tetris->columns,
            base_level->next->matrix);
    }
