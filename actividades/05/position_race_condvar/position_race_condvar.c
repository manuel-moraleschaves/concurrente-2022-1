...
typedef struct shared_data {
    ...
    pthread_mutex_t can_access_condition;
...
int create_threads(size_t thread_count) {
    ...
    if (threads) {
        ...
        if (private_data && shared_data) {
            ...
            shared_data->condition = 0;
            pthread_mutex_init(&shared_data->can_access_position, NULL);
            pthread_mutex_init(&shared_data->can_access_condition, NULL);
            ...
            pthread_mutex_destroy(&shared_data->can_access_condition);
            pthread_cond_destroy(&shared_data->condvar);
            free(shared_data);
...
void* run(void* params) {
    ...
    pthread_mutex_lock(&shared_data->can_access_condition);
    shared_data->condition++;
    if (shared_data->condition < shared_data->thread_count) {
        pthread_cond_wait(&shared_data->condvar, &shared_data->can_access_condition);
    } else {
        pthread_cond_broadcast(&shared_data->condvar);
    }
    pthread_mutex_unlock(&shared_data->can_access_condition);
